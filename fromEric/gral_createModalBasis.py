...
def createModalBasis( miaHO, miaTT, delta, fullNoll, Z2S, nfilt ):
    ...
    <pistonProj> : the vector that extracts the value of the piston from a set of voltages of the HODM.
...
def pseudoOpenLoopReconstruction(slopesdata, voltsdataHO, voltsdataTT, miaHO, miaTT, Fs, latency):
    """

    Input arguments :
    <slopesdata>  : set of N frames of slopes (i.e. centroids), (120xN)
    <voltsdataHO> : set of N frames of volts (i.e. DM commands) synchronized with slopes, (60xN)
    <voltsdataTT> : set of N frames of volts (i.e. TT commands) synchronized with slopes, (2xN)
...
def modalControlOptimization( slopesdata, voltsdataHO, voltsdataTT, miaHO, miaTT, S2M, M2V, gmax, Fs, latency, BP ):
    """
    Input arguments :
    <slopesdata> : set of N frames of slopes (i.e. centroids), (120xN)
    <voltsdataHO> : set of N frames of volts (i.e. DM commands) synchronized with slopes, (60xN)
    ...
    slopes = pseudoOpenLoopReconstruction(slopesdata, voltsdataHO, voltsdataTT, miaHO, miaTT, Fs, latency)
...
(nframes, ns) = s.shape
s = s + np.random.randn( nframes, ns )*40   # add random numbers on slopes to simulate noise
...
optiGain = modalControlOptimization( s, vHO, vTT, miaHO, miaTT, S2M, M2V, 0.8, Fs, latency, BP )
